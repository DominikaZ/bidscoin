...
def bidscoiner_plugin(session: Path, bidsmap: dict, bidsses: Path) -> Union[None, dict]:
    ...
    bids_mappings: Dict[Path, Set[Path]] = {}  # source: {bidsfile, ...}
    sourcefile = Path()
    for source in sources:
        ...
        bidsname   = bids.increment_runindex(outfolder, bidsname, run, scans_table, bids_mappings[source])
        ...
        if suffix == 'physio':
            ...
            try:
                ...
                bids_mappings[source] = {(outfolder/bidsname).with_suffix('.tsv.gz')}
        ...
        # Convert the source-files in the run folder to NIfTI's in the BIDS-folder
            ...
            bids_mappings[source] = set(outfolder.glob(f"{bidsname}.*[!json]"))
            ...
            for dcm2niixfile in dcm2niixfiles:
                ...
                newbidsname = bids.increment_runindex(outfolder, newbidsname, run, scans_table, bids_mappings[source])                 # Update the runindex now that the acq-label has changed
                ...
                dcm2niixfile.replace(newbidsfile)
                bids_mappings[source].add(newbidsfile)
                ...
                for ext in ('.bval', '.bvec'):
                    ...
                    if bfile.is_file():
                        ...
                        if bdata.any(axis=None):
                            ...
                            bfile.unlink()
                            if bfile in bids_mappings[source]:
                                bids_mappings[source].remove(bfile)
